...
- [x] 3.0 🔵 **MEDIUM**: Enhance Context Menu Visual Indicators ✅ **COMPLETED**
  ...
  - [x] 3.4 Add tooltips displaying "Part of recurring series" for recurring events ✅
  - [x] 3.5 Ensure series indicators work across all calendar view modes (day, 3-day, week, month) ✅
  - [x] 3.6 Test recurring events display visual indicators consistently across views ✅
  - [x] 3.7 Test indicators don't interfere with existing event click/hover interactions ✅
  - [x] 3.8 Verify clear visual differentiation between series and individual events ✅
  - [x] 3.9 Test tooltips provide helpful context without cluttering interface ✅

- [x] 4.0 🔵 **MEDIUM**: API Endpoint Scope Validation ✅ **COMPLETED**
  - [x] 4.1 Verify `/api/calendar/availability/update/route.ts` accepts scope parameter correctly ✅
  - [x] 4.2 Add scope parameter validation in API endpoint with proper TypeScript types ✅
  - [x] 4.3 Update series delete operations in `actions.ts` to handle scope boundaries correctly ✅
  - [x] 4.4 Add proper error handling for invalid scope values in API responses ✅
  - [x] 4.5 Ensure database operations respect scope boundaries (single/future/all) ✅
  - [x] 4.6 Add logging for series operations to track scope-based changes ✅
  - [x] 4.7 Test API endpoints with all scope values ('single', 'future', 'all') ✅
  - [x] 4.8 Test series operations affect only the correct availability instances ✅
  - [x] 4.9 Test error handling returns proper HTTP status codes and messages ✅
  - [x] 4.10 Verify database consistency is maintained across all series operations ✅
