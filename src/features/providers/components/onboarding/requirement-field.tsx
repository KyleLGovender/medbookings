...
export function RequirementField({ requirement, onChange }: RequirementFieldProps) {
  ...
  switch (requirement.validationType) {
    ...
    case "DOCUMENT":
      return <DocumentUploader onUpload={handleChange} acceptedFormats={requirement.acceptedFormats} />
    ...
    case "PAST_DATE":
      return (
        <Input
          ...
          value={value || ""}
          onChange={(e) => handleChange(e.target.value)}
          min={requirement.validationType === "FUTURE_DATE" ? new Date().toISOString().split("T")[0] : undefined}
          max={requirement.validationType === "PAST_DATE" ? new Date().toISOString().split("T")[0] : undefined}
        />
      )
