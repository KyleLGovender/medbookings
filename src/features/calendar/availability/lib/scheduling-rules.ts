import { addMinutes, startOfHour, startOfDay, setMinutes, setHours } from 'date-fns';
import { SchedulingRule, SchedulingRuleConfig } from '../types';
...
export function validateSchedulingRuleConfig(config: SchedulingRuleConfig): {
  ...
  switch (config.rule) {
    case SchedulingRule.CUSTOM_INTERVAL:
      ...
      if (config.interval && config.interval > 1440) { // 24 hours
        errors.push('Custom interval cannot exceed 24 hours (1440 minutes)');
      }
      break;
      
    case SchedulingRule.FIXED_INTERVAL:
      const hasAlignment = config.alignToHour || config.alignToHalfHour || config.alignToQuarterHour;
      if (!hasAlignment) {
  ...
  switch (rule) {
    ...
    case SchedulingRule.CUSTOM_INTERVAL:
      if (ruleConfig?.interval) {
        ...
        const minutesFromStart = Math.floor((fromTime.getTime() - startOfDayTime.getTime()) / (60 * 1000));
        const nextInterval = Math.ceil(minutesFromStart / ruleConfig.interval) * ruleConfig.interval;
