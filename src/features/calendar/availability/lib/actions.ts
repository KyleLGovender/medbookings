...
export async function createAvailability(
  ...
  try {
    ...
    if (!canCreateForProvider && validatedData.organizationId) {
      ...
      return { success: false, error: 'Can only create availability for your own provider account' };
    }

    // Determine initial status based on context
    const isProviderCreated = currentUser.id === validatedData.serviceProviderId;
    const initialStatus = isProviderCreated ? AvailabilityStatus.ACTIVE : AvailabilityStatus.PENDING;

    // Determine billing entity if not specified
    const billingEntity = validatedData.billingEntity || (
    ...
    if (!isProviderCreated && availability.status === AvailabilityStatus.PENDING) {
      try {
        await notifyAvailabilityProposed(
...
export async function updateAvailability(
  ...
  try {
    ...
    if (validatedData.recurrencePattern !== undefined) updateData.recurrencePattern = validatedData.recurrencePattern;
