...
const defaultValues: Partial<BookingFormValues> = {
  bookingType: BookingTypeSchema.enum.GUEST_SELF,
...
export function BookingForm({
  ...
  useEffect(() => {
    form.setValue('clientId', bookingType === 'USER_SELF' ? userId : undefined);
  ...
  const onSubmit = async (values: BookingFormValues) => {
    ...
    try {
      ...
      if (values.bookingType === 'USER_SELF') {
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent>
                    {Object.values(BookingTypeSchema.enum).map((type) => (
                      <SelectItem key={type} value={type}>
                        {type === 'USER_SELF' && 'Book for myself'}
                        {type === 'USER_GUEST' && 'Book for someone else'}
                        {type === 'GUEST_SELF' && 'Guest booking'}
                        {type === 'PROVIDER_GUEST' && 'Provider booking'}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
