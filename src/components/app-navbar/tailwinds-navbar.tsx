...
import AuthButton from '@/components/app-navbar/auth-button-tailwinds';
...
export default function TailwindsNavbar() {
  const { data: session } = useSession();
  const pathname = usePathname();
  const menuItems = [{ label: 'Home', href: '/' }];
  const profileMenuItems = [{ label: 'Profile', href: '/profile' }, { label: 'Settings', href: '/settings' }];
        ...
        <div className="flex h-16 justify-between">
          ...
          <div className="flex items-center">
            ...
            <div className="hidden md:ml-4 md:flex md:shrink-0 md:items-center">
              <button
                type="button"
                className="relative rounded-full bg-white p-1 text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-blue500 focus:ring-offset-2"
              >
      ...
      <DisclosurePanel className="md:hidden">
        ...
        <div className="border-t border-gray-200 pb-3 pt-4">
          <div className="flex items-center px-4 sm:px-6">
            ...
            <div className="ml-3">
              <div className="text-base font-medium text-gray-800">{session?.user?.name || 'Guest'}</div>
              <div className="text-sm font-medium text-gray-500">{session?.user?.email || 'Guest'}</div>
            </div>
            <button
              type="button"
