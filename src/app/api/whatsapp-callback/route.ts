...
export async function POST(request: NextRequest) {
  logger.info('WhatsApp callback POST request received', { url: request.url });
  ...
  if (!authToken) {
    logger.error('WhatsApp callback - Twilio Auth Token not configured');
  ...
  try {
    ...
    if (!isValidRequest) {
      logger.warn('WhatsApp callback - Invalid Twilio signature');
    ...
    if (!fromNumberRaw) {
      logger.warn('WhatsApp callback - Missing From in payload');
      return NextResponse.json({ error: 'Missing required payload fields' }, { status: 400 });
    }

    // Check if it's an interactive reply we care about
    if (!buttonPayload) {
      logger.info('WhatsApp callback - Received non-button message, ignoring', {
        from: sanitizePhone(fromNumberRaw),
      });
    ...
    if (!fromNumberNormalized) {
      logger.warn('WhatsApp callback - Could not normalize From number', {
        from: sanitizePhone(fromNumberRaw),
      });
      return NextResponse.json({ error: 'Invalid From number format' }, { status: 400 });
    }

    logger.info('WhatsApp callback - Received button click', {
      from: sanitizePhone(fromNumberNormalized),
      hasPayload: !!buttonPayload,
    });
    ...
    if (buttonPayload.startsWith(VCARD_REQUEST_PREFIX)) {
      ...
      if (!bookingId) {
        logger.warn('WhatsApp callback - Invalid VCARD request payload', {
          from: sanitizePhone(fromNumberNormalized),
        });
        return NextResponse.json({ error: 'Invalid button payload format' }, { status: 400 });
      }

      logger.info('WhatsApp callback - Handling VCARD request', {
        bookingId,
        from: sanitizePhone(fromNumberNormalized),
      });
      ...
      if (!booking) {
        logger.error('WhatsApp callback - Booking not found for VCARD request', { bookingId });
      ...
      if (!providerWhatsappNormalized || fromNumberNormalized !== providerWhatsappNormalized) {
        logger.warn('WhatsApp callback - Number mismatch for VCARD request', {
          from: sanitizePhone(fromNumberNormalized),
          bookingId,
        });
      ...
      logger.info('WhatsApp callback - vCard sent successfully', { bookingId: booking.id });
    ...
    // Add more `else if (buttonPayload.startsWith(...))` blocks here for other button actions
      ...
      logger.info('WhatsApp callback - Unhandled button payload', {
        from: sanitizePhone(fromNumberNormalized),
      });
      return NextResponse.json({}, { status: 200 }); // Acknowledge receipt
    }
  } catch (error) {
    logger.error('WhatsApp callback - Error processing webhook', {
...
export async function GET(request: NextRequest) {
  ...
  logger.info('WhatsApp callback GET request', {
    ip,
    userAgent,
