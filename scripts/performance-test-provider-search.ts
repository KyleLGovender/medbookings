...
async function performanceTestProviderSearch() {
  ...
  if (typeIds.length > 0) {
    ...
    results.push(test3);
    console.log(`   ⏱️  Average: ${test3.averageTime.toFixed(2)}ms | Records: ${test3.recordCount}`);
  }
  ...
  if (allTypeIds.length > 1) {
    ...
    results.push(test4);
    console.log(`   ⏱️  Average: ${test4.averageTime.toFixed(2)}ms | Records: ${test4.recordCount}`);
  }
  ...
  if (typeIds.length > 0) {
    ...
    results.push(test5);
    console.log(`   ⏱️  Average: ${test5.averageTime.toFixed(2)}ms | Records: ${test5.recordCount}`);
  }
  ...
  if (typeIds.length > 0) {
    ...
    results.push(test6);
    console.log(`   ⏱️  Average: ${test6.averageTime.toFixed(2)}ms | Records: ${test6.recordCount}`);
  }

  // Test 7: Provider type statistics
  console.log('🔍 Test 7: Provider type statistics');
  const test7 = await measurePerformance(
    'Provider type stats',
  ...
  const test8a = await measurePerformance(
    'First call (uncached)',
    () => getProviderTypeStats(),
    1
  );
  ...
  const test8b = await measurePerformance(
    'Second call (cached)',
    () => getProviderTypeStats(),
    1
  );
  console.log(`      ⏱️  Time: ${test8b.averageTime.toFixed(2)}ms`);
  console.log(`      📈 Speed improvement: ${(test8a.averageTime / test8b.averageTime).toFixed(1)}x faster`);
  ...
  results.forEach(result => {
    console.log(
      ...
      result.averageTime.toFixed(2).padEnd(15) + 
      result.recordCount
    );
  });
