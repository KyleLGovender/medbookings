...
test.describe('Deletion and Cleanup Journeys', () => {
  ...
  test.describe('Availability Deletion', () => {
    ...
    test('should delete single availability slot', async ({ page }) => {
      ...
      const availabilitySlot = page.locator(`[data-testid="availability-slot-${TEST_AVAILABILITY_DATA.date}"]`);
      ...
      // Verify deletion
      await expect(page.locator('[data-testid="availability-deleted-success"]')).toContainText('Availability deleted successfully');
    ...
    test('should delete recurring availability series', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="series-deleted-success"]')).toContainText('Recurring availability series deleted successfully');
    ...
    test('should delete single occurrence from recurring series', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="occurrence-deleted-success"]')).toContainText('Availability occurrence deleted');
    ...
    test('should prevent deletion of booked availability', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="deletion-error"]')).toContainText('Cannot delete availability with existing bookings');
      await expect(page.locator('[data-testid="bookings-count"]')).toContainText('2 active bookings');
    ...
    test('should bulk delete multiple availability slots', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="bulk-delete-success"]')).toContainText('3 availability slots deleted successfully');
  ...
  test.describe('Provider Profile Deletion', () => {
    ...
    test('should delete provider profile completely', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="deletion-warning"]')).toContainText('This action cannot be undone');
      await expect(page.locator('[data-testid="deletion-consequences"]')).toContainText('All your availability, bookings, and profile data will be permanently deleted');
      ...
      await expect(page.locator('[data-testid="delete-confirmation-text"]')).toContainText('Type DELETE to confirm');
      ...
      await expect(page.locator('[data-testid="profile-deleted-message"]')).toContainText('Your provider profile has been deleted successfully');
    });
    ...
    test('should require exact confirmation text', async ({ page }) => {
      ...
      // Should show error
      await expect(page.locator('[data-testid="confirmation-error"]')).toContainText('Please type DELETE exactly as shown');
    ...
    test('should prevent deletion with active bookings', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="deletion-blocked-error"]')).toContainText('Cannot delete provider profile with active bookings');
      await expect(page.locator('[data-testid="active-bookings-count"]')).toContainText('3 active bookings');
      
      // Should show next steps
      await expect(page.locator('[data-testid="cancel-bookings-first"]')).toContainText('You must cancel all active bookings first');
    });
    ...
    test('should export data before deletion', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="export-success"]')).toContainText('Data exported successfully');
    });
  });
  ...
  test.describe('Organization Deletion', () => {
    ...
    test('should delete organization with confirmation', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="organization-deletion-warning"]')).toContainText('This will permanently delete the organization');
      await expect(page.locator('[data-testid="deletion-impact"]')).toContainText('All members will lose access');
      ...
      await expect(page.locator('[data-testid="organization-name-confirmation"]')).toContainText(testOrganization.name);
      ...
      await expect(page.locator('[data-testid="organization-deleted-success"]')).toContainText('Organization deleted successfully');
    });
    ...
    test('should require exact organization name', async ({ page }) => {
      ...
      // Should show error
      await expect(page.locator('[data-testid="name-mismatch-error"]')).toContainText('Organization name does not match');
    });
    ...
    test('should prevent deletion with active members', async ({ page }) => {
      ...
      // Should show blocking error
      await expect(page.locator('[data-testid="deletion-blocked"]')).toContainText('Cannot delete organization with active members');
      await expect(page.locator('[data-testid="members-count"]')).toContainText('5 active members');
      await expect(page.locator('[data-testid="providers-count"]')).toContainText('3 connected providers');
      
      // Should show required actions
      await expect(page.locator('[data-testid="required-actions"]')).toContainText('Remove all members and disconnect all providers first');
    });
    ...
    test('should transfer organization ownership before deletion', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="ownership-transferred"]')).toContainText('Ownership transferred successfully');
    ...
    test('should export organization data before deletion', async ({ page }) => {
      ...
      expect(download.suggestedFilename()).toContain(testOrganization.name.toLowerCase().replace(/\s+/g, '-'));
      
      // Should show export confirmation
      await expect(page.locator('[data-testid="organization-export-success"]')).toContainText('Organization data exported successfully');
    });
  });

  test.describe('Cascade Deletion Verification', () => {
    test('should verify all related data is cleaned up after provider deletion', async ({ page }) => {
      // Mock provider authentication
      await page.route('**/api/auth/session', async (route) => {
        const json = {
          user: {
    ...
    test('should verify organization deletion cleans up all related resources', async ({ page }) => {
      // Mock organization owner authentication
      await page.route('**/api/auth/session', async (route) => {
        const json = {
          user: {
      ...
      await expect(page.locator('[data-testid="organization-not-found"]')).toBeVisible()
