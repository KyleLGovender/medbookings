...
test.describe('Provider Calendar Management', () => {
  ...
  test.describe('Availability Creation', () => {
    ...
    test('should create single availability slot', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="availability-created-success"]')).toContainText('Availability created successfully');
    ...
    test('should create recurring availability slots', async ({ page }) => {
      ...
      // Should show success message
      await expect(page.locator('[data-testid="recurring-availability-success"]')).toContainText('Recurring availability created successfully');
    ...
    test('should validate availability form', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="availability-date-error"]')).toContainText('Date is required');
      await expect(page.locator('[data-testid="availability-start-time-error"]')).toContainText('Start time is required');
      await expect(page.locator('[data-testid="availability-end-time-error"]')).toContainText('End time is required');
      await expect(page.locator('[data-testid="availability-service-error"]')).toContainText('Service is required');
    });
    ...
    test('should prevent past date availability', async ({ page }) => {
      ...
      // Should show validation error
      await expect(page.locator('[data-testid="past-date-error"]')).toContainText('Cannot create availability for past dates');
    });
  });
  ...
  test.describe('Availability Management', () => {
    ...
    test('should view availability details', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="availability-service"]')).toContainText(TEST_AVAILABILITY_DATA.service);
      await expect(page.locator('[data-testid="availability-time"]')).toContainText(`${TEST_AVAILABILITY_DATA.startTime} - ${TEST_AVAILABILITY_DATA.endTime}`);
      await expect(page.locator('[data-testid="availability-type"]')).toContainText(TEST_AVAILABILITY_DATA.type);
    ...
    test('should edit availability slot', async ({ page }) => {
      ...
      // Should show success message
      await expect(page.locator('[data-testid="availability-updated-success"]')).toContainText('Availability updated successfully');
      
      // Should update display
      await availabilitySlot.click();
      await expect(page.locator('[data-testid="availability-time"]')).toContainText('10:00 - 16:00');
    });
    ...
    test('should delete availability slot', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="delete-confirmation-message"]')).toContainText('Are you sure you want to delete this availability slot?');
  ...
  test.describe('Availability Conflicts', () => {
    test('should detect overlapping availability', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="availability-conflict-warning"]')).toContainText('This time slot overlaps with existing availability');
    ...
    test('should detect Google Calendar conflicts', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="google-calendar-conflict"]')).toContainText('This conflicts with your Google Calendar event: Doctor Appointment');
