...
import { REJECTION_REASONS, APPROVAL_NOTES } from '../../fixtures/test-data';
...
test.describe('Provider Approval Workflow', () => {
  ...
  test.describe('Admin Provider Review', () => {
    ...
    test('should display pending providers for admin review', async ({ page }) => {
      ...
      await expect(providerCard.locator('[data-testid="provider-name"]')).toContainText('Dr. John Doe');
      await expect(providerCard.locator('[data-testid="provider-status"]')).toContainText('Pending Approval');
    });
    ...
    test('should show provider details for admin review', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="provider-email"]')).toContainText('provider@test.com');
    ...
    test('should successfully approve provider', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="approval-success"]')).toContainText('Provider approved successfully');
    ...
    test('should successfully reject provider with reason', async ({ page }) => {
      ...
      // Should show success message
      await expect(page.locator('[data-testid="rejection-success"]')).toContainText('Provider rejected');
      
      // Status should be updated
      await expect(page.locator('[data-testid="provider-status"]')).toContainText('Rejected');
      
      // Should show rejection reason
      await expect(page.locator('[data-testid="rejection-reason-display"]')).toContainText(REJECTION_REASONS.provider);
    });
    ...
    test('should require rejection reason', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="rejection-reason-error"]')).toContainText('Rejection reason is required');
    ...
    test('should filter providers by status', async ({ page }) => {
      ...
      await expect(page.locator(`[data-testid="provider-${approvedProvider.id}"]`)).not.toBeVisible();
    ...
    test('should search providers by name', async ({ page }) => {
      ...
      await expect(page.locator(`[data-testid="provider-${testProvider.id}"]`)).not.toBeVisible();
      await expect(page.locator('[data-testid="no-providers-found"]')).toContainText('No providers found');
    });
  });
  ...
  test.describe('Provider Notifications', () => {
    ...
    test('should show approval notification to provider', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="provider-status"]')).toContainText('Approved');
      await expect(page.locator('[data-testid="approval-notification"]')).toContainText('Congratulations! Your provider application has been approved');
    ...
    test('should show rejection notification with reason', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="provider-status"]')).toContainText('Rejected');
      await expect(page.locator('[data-testid="rejection-notification"]')).toContainText('Unfortunately, your provider application was not approved');
      await expect(page.locator('[data-testid="rejection-reason"]')).toContainText(REJECTION_REASONS.provider);
