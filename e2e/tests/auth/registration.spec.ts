...
test.describe('User Registration Flow', () => {
  ...
  test.describe('Provider Registration', () => {
    test('should complete provider registration successfully', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="registration-success"]')).toContainText('Registration submitted successfully');
      
      // Should redirect to provider profile page
      await page.waitForURL('/providers/current');
      
      // Verify provider data is displayed
      await expect(page.locator('[data-testid="provider-name"]')).toContainText(`${TEST_PROVIDER_DATA.firstName} ${TEST_PROVIDER_DATA.lastName}`);
      await expect(page.locator('[data-testid="provider-status"]')).toContainText('Pending Approval');
    });
    ...
    test('should handle requirement document upload', async ({ page }) => {
      ...
      await uploadRequirementDocument(page, 'Professional Insurance', 'e2e/fixtures/files/insurance-certificate.pdf');
      ...
      await page.waitForURL('/providers/current');
      await expect(page.locator('[data-testid="requirement-Medical License-status"]')).toContainText('Submitted');
      await expect(page.locator('[data-testid="requirement-Professional Insurance-status"]')).toContainText('Submitted');
    });
    ...
    test('should allow editing provider information after registration', async ({ page }) => {
      ...
      // Verify update
      await expect(page.locator('[data-testid="save-success"]')).toContainText('Changes saved successfully');
  ...
  test.describe('Organization Registration', () => {
    test('should complete organization registration successfully', async ({ page }) => {
      ...
      // Should show success message
      await expect(page.locator('[data-testid="registration-success"]')).toContainText('Organization registered successfully');
      
      // Should redirect to organization page
      await page.waitForURL(/\/organizations\/\d+/);
      
      // Verify organization data is displayed
      await expect(page.locator('[data-testid="organization-name"]')).toContainText(TEST_ORGANIZATION_DATA.name);
      await expect(page.locator('[data-testid="organization-status"]')).toContainText('Pending Approval');
    });
    ...
    test('should validate organization form fields', async ({ page }) => {
      ...
      // Should show validation errors
      await expect(page.locator('[data-testid="organizationName-error"]')).toContainText('Organization name is required');
      await expect(page.locator('[data-testid="organizationPhone-error"]')).toContainText('Phone number is required');
      await expect(page.locator('[data-testid="organizationEmail-error"]')).toContainText('Email is required');
    });
    ...
    test('should handle organization location setup', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="location-success"]')).toContainText('Location added successfully');
    });
  });
  ...
  test.describe('Invitation Acceptance', () => {
    test('should accept provider invitation to organization', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="invitation-type"]')).toContainText('Organization Invitation');
      await expect(page.locator('[data-testid="organization-name"]')).toContainText('Test Medical Clinic');
      ...
      // Should show success message
      await expect(page.locator('[data-testid="acceptance-success"]')).toContainText('Invitation accepted successfully');
    ...
    test('should handle invalid invitation tokens', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="invitation-error"]')).toContainText('Invitation not found or expired');
    ...
    test('should reject invitation', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="rejection-success"]')).toContainText('Invitation rejected');
