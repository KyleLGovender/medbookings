...
- [ ] 1.0 🔴 **CRITICAL**: Fix Authorization Pattern in Calendar Actions
  - [ ] 1.1 Fix `createAvailability` authorization check at line 54 in `src/features/calendar/lib/actions.ts`
  - [ ] 1.2 Add Provider lookup query: `await prisma.provider.findUnique({ where: { userId: currentUser.id } })`
  - [ ] 1.3 Update authorization check to compare Provider IDs: `currentUserProvider?.id === validatedData.providerId`
  - [ ] 1.4 Fix `getAvailabilityById` authorization check at line 310 in `src/features/calendar/lib/actions.ts`
  - [ ] 1.5 Fix `updateAvailability` authorization check at line 479 in `src/features/calendar/lib/actions.ts`
  - [ ] 1.6 Fix `deleteAvailability` authorization check at line 630 in `src/features/calendar/lib/actions.ts`
  - [ ] 1.7 Fix `cancelAvailability` authorization check at line 727 in `src/features/calendar/lib/actions.ts`
  - [ ] 1.8 Consolidate Provider lookups to avoid duplicate database queries across all functions
  - [ ] 1.9 Add proper error handling when user doesn't have Provider record
...
- [ ] 2.0 🔴 **CRITICAL**: Fix Authorization Pattern in Workflow Service
  - [ ] 2.1 Fix authorization check at line 54 in `src/features/calendar/lib/workflow-service.ts`
  - [ ] 2.2 Fix authorization check at line 178 in `src/features/calendar/lib/workflow-service.ts`
  - [ ] 2.3 Fix authorization check at line 280 in `src/features/calendar/lib/workflow-service.ts`
  - [ ] 2.4 Fix authorization check at line 401 in `src/features/calendar/lib/workflow-service.ts`
  - [ ] 2.5 Update all workflow functions to fetch Provider record before authorization checks
  - [ ] 2.6 Ensure workflow functions use Provider ID comparisons: `currentUserProvider?.id === targetProviderId`
  - [ ] 2.7 Add error handling for users without Provider records in workflow operations
...
- [ ] 3.0 🟡 **HIGH**: Fix Field Name Inconsistencies
  - [ ] 3.1 Update `serviceProviderId` reference to `providerId` at line 360 in `src/features/calendar/lib/actions.ts`
  - [ ] 3.2 Update `serviceProviderId` reference to `providerId` at line 422 in `src/features/calendar/lib/actions.ts`
  - [ ] 3.3 Search codebase for any remaining `serviceProviderId` references in calendar feature
  - [ ] 3.4 Update related type definitions in `src/features/calendar/types/types.ts` if needed
